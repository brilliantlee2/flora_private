...
def write_matrix(output_path, genes, barcodes, umi_sets):
    """ """
    rows = list(genes)
    rows.sort()
    ...
    for (gene, barcode), umis in tqdm(umi_sets.items()):
        gene_cell_counts[gene][barcode] = len(umis)

    with open(output_path, "w", newline="") as handle:
        writer = csv.writer(handle, delimiter="\t")
        writer.writerow(["gene", *cols])
        for gene in tqdm(rows):
            counts = gene_cell_counts.get(gene, {})
            writer.writerow([gene, *[counts.get(barcode, 0) for barcode in cols]])
...
def process_bam_entries(args):
    ...
    logger.info(f"Writing gene expression matrix to {args.output}")
    write_matrix(args.output, genes, barcodes, umi_sets)
