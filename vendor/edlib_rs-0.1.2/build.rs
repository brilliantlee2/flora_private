...
fn main() {
    ...
    let mut libdir = String::from("cargo:rustc-link-search=native=");
    libdir.push_str(out_path.to_str().unwrap());
    libdir.push_str("/lib");
    println!("{}", libdir);
    let mut libdir64 = String::from("cargo:rustc-link-search=native=");
    libdir64.push_str(out_path.to_str().unwrap());
    libdir64.push_str("/lib64");
    println!("{}", libdir64);
