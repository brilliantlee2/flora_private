...
The packaging script copies `environment.runtime.yml` into the archive as `environment.yml`, uses the public README templates, compiles Python files to Python 3.11 bytecode, and excludes Rust/Python source.
...
file target/release/flora
ldd target/release/flora
./target/release/flora --version
./target/release/flora glycine --help
bash run_all.sh -h
cat PYTHON_ABI.txt
...
`file target/release/flora` should report an ELF x86-64 executable and `stripped`. Do not publish an archive whose main executable is reported as `not stripped`.
