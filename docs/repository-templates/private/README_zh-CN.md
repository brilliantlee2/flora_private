...
run_all.sh                    单物种流程
run_all_mixed_species.sh      mixed-species 流程
...
打包脚本会将 `environment.runtime.yml` 作为发行包的 `environment.yml`，使用公开 README 模板，将 Python 编译为 Python 3.11 字节码，并排除 Rust/Python 源码。
...
`file target/release/flora` 应显示 ELF x86-64 可执行文件且包含 `stripped`。如果显示 `not stripped`，不要发布该压缩包。
